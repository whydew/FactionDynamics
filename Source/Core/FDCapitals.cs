...
namespace FactionDynamics
{
    ...
    /// <summary>
    /// Draws the actual capital marker: a gold box around the settlement's expanded icon with its
    /// name underneath.
    ///
    /// The tint and the ExpandMore size bump above help, but on a planet carrying two hundred
    /// settlement icons they are not enough on their own to answer "where is their capital?" - the
    /// eye has nothing to search for. A hard outline plus a name is unmistakable, and with one
    /// capital per faction there are only a handful of them on screen.
    ///
    /// This runs as a postfix on vanilla's own expanded-icon pass, so it draws on top of every
    /// icon and inherits the same zoom behaviour: it fades in with the icons and disappears
    /// entirely when zoomed in close, where settlement names are legible anyway.
    /// </summary>
    [HarmonyPatch(typeof(ExpandableWorldObjectsUtility),
        nameof(ExpandableWorldObjectsUtility.ExpandableWorldObjectsOnGUI))]
    public static class Patch_CapitalWorldMarker
    {
        private static readonly Color MarkerColor = new Color(1f, 0.87f, 0.45f);

        [HarmonyPostfix]
        public static void Postfix()
        {
            if (Event.current == null || Event.current.type != EventType.Repaint) return;
            if (!DebugViewSettings.drawWorldObjects) return;
            ...
            {
                Settlement s = all[i];
                if (s == null || !FDCapitals.IsCapital(s)) continue;

                // Match vanilla's own visibility rules for this pass, so the marker never floats
                // over a settlement whose icon is not actually being drawn.
                if (s.def == null || !s.def.expandingIcon) continue;
                if (ExpandableWorldObjectsUtility.HiddenByRules(s)) continue;
                if (s.HiddenBehindTerrainNow()) continue;

                float pct = ExpandableWorldObjectsUtility.TransitionPct(s);
                if (pct <= 0.01f) continue;

                Rect icon = ExpandableWorldObjectsUtility.ExpandedIconScreenRect(s, 1f);
                var ring = new Rect(icon.x - 4f, icon.y - 4f, icon.width + 8f, icon.height + 8f);

                GUI.color = new Color(MarkerColor.r, MarkerColor.g, MarkerColor.b, pct);
                Widgets.DrawBox(ring, 2, BaseContent.WhiteTex);

                var label = new Rect(ring.center.x - 90f, ring.yMax + 1f, 180f, 18f);
                Widgets.Label(label, s.Label);
            }
